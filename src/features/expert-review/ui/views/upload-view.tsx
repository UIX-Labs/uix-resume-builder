...
export function UploadView({ onUpload, onSignIn, onClose }: UploadViewProps) {
  ...
  return (
    <div className="relative flex w-full min-h-[min(100dvh-2rem,653px)] sm:min-h-[653px] p-6 sm:p-8 md:p-12 bg-transparent text-white overflow-hidden">
      ...
      <div className="relative z-10 flex w-full flex-1 min-w-0 gap-6 md:gap-10">
        <section className="flex flex-col justify-center gap-6 sm:gap-8 max-w-3xl mx-4 sm:ml-8 md:ml-16 flex-1 min-w-0">
          <div className="flex flex-col gap-4 sm:gap-5">
            <h2
              className="text-expert-heading font-semibold leading-[1] tracking-[-0.18px] max-w-lg"
              style={GEIST_FONT}
            >
              <span className="text-[28px] sm:text-[36px] md:text-[44px] block whitespace-nowrap">Stop Guessing.</span>
              <span className="text-[32px] sm:text-[44px] md:text-[56px] block whitespace-nowrap">
                Get Expert Feedback.
              </span>
            </h2>
            <p
