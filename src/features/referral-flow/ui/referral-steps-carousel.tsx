...
interface Step {
  id?: string; // Optional ID for cleaner keys
  icon: string;
  title: string;
  description: string;
  threadImage?: string;
  ...
  return (
    <div className="w-full">
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex">
          {steps.map((step, index) => {
            const stepKey = step.id || `carousel-step-${index}-${step.title.slice(0, 10)}`;

            return (
              <div key={stepKey} className="flex-[0_0_100%] min-w-0 px-4">
                <div className="flex flex-col items-center gap-2 relative pt-16 z-10">
                  {step.threadImage && (
                    <>
                      ...
                      {(step.threadImage === 'both' || step.threadImage === 'right') && (
      ...
      <div className="flex justify-center mt-6 gap-2">
        {steps.map((step, index) => {
          const dotKey = `dot-${step.id || index}`;
