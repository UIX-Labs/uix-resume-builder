...
export function renderField(
  ...
  const errorSuggestions = fieldPath
    ? getFieldSuggestions(suggestedUpdates, itemId, fieldPath)
    : [];
  const errorBgColor = isThumbnail
    ? ""
    : getSuggestionBackgroundColor(errorSuggestions);
      ...
      <div className={field.className}>
        {field.heading && (
          <div className={field.heading.className}>
            <p>
              {resolvePath(data, field.heading.path, field.heading.fallback)}
            </p>
  ...
  if (field.type === "inline-group") {
    ...
    const content = renderedItems.map(
      (
        { element, idx }: { element: React.ReactNode; idx: number },
        arrayIdx: number
      ) => (
        <React.Fragment key={idx}>
    ...
    return <span className={cn(errorBgColor)}>{content}</span>;
  }
  ...
  if (field.type === "icon") {
    ...
    return (
      <IconComponent size={field.size || 16} className={field.className} />
    );
  }
  ...
  if (field.type === "image") {
    ...
    return (
      <img
        src={imageSrc}
        crossOrigin={
          isThumbnail && isExternalUrl(src) ? "anonymous" : undefined
        }
        alt={field.alt || "Image"}
  ...
  if (field.type === "skillLevel") {
    ...
    return (
      <div className={cn("flex gap-1", field.className, errorBgColor)}>
  ...
  if (field.type === "duration") {
    ...
    if (duration.startDate && duration.endDate) {
      ...
      return (
        <span
          className={cn(field.className, errorBgColor)}
        >{`${start} - ${end}`}</span>
      );
    }
...
export function renderItemWithRows(
  ...
  return template.rows.map((row: any, rowIdx: number) => {
    ...
    const hasBreakableCell = row.cells.some(
      (cell: any) => cell.break === true || cell.breakable === true
    );
    const isRowBreakable =
      row.break === true || row.breakable === true || hasBreakableCell;
  ...
  return template.fields.map((field: any, idx: number) => (
    <div
      ...
      data-has-breakable-content={
        field.break || field.breakable ? "true" : undefined
      }
    >
