...
export function renderHeaderSection(
  ...
  const hasGenericFields = Object.values(fields).some(
    (field: any) =>
      field?.type && ["image", "group", "text"].includes(field.type)
  );
  ...
  const professionalSummarySuggestions =
    data["professionalSummary"]?.suggestedUpdates;
  const hasValidPersonalDetailsSuggestions = hasPendingSuggestions(
    personalDetailsSuggestions
  );
  const hasValidSummarySuggestions = hasPendingSuggestions(
    professionalSummarySuggestions
  );
  const hasValidSuggestions =
    hasValidPersonalDetailsSuggestions || hasValidSummarySuggestions;
  ...
  const personalDetailsItemId =
    personalDetailsItem?.itemId || personalDetailsItem?.id;
  ...
  return (
    ...
    >
      ...
      {fields.nameTitle ? (
        <div className={fields.nameTitle.className}>
          {fields.name && (
            <p
              className={cn(
                fields.name.className /*, getFieldErrorBgColor('fullName')*/
              )}
            >
          ...
          {fields.title?.path && (
            <p
              className={cn(
                fields.title.className /*, getFieldErrorBgColor('jobTitle')*/
              )}
            >
          ...
          {fields.description?.path && (
            <div
              className={cn(
                fields.description
                  .className /*, getFieldErrorBgColor('description')*/
              )}
          ...
          {(() => {
            ...
            return validItems.map((entry: any, arrayIdx: number) => {
              ...
              if (item.type === "link") {
                ...
                return (
                  <span key={originalIdx}>
                    ...
                    <a
                      href={href}
                      className={cn(item.className /*, errorBgColor*/)}
                      {...linkProps}
                    >
                      {value}
                    </a>
              ...
              return (
                <span key={originalIdx}>
                  {showSeparator && fields.contact.separator}
                  <span className={cn(item.className /*, errorBgColor*/)}>
                    {value}
                  </span>
                </span>
              );
      ...
      {fields.address && (
        <p
          className={cn(
            fields.address.className /*, getFieldErrorBgColor('address')*/
          )}
        >
