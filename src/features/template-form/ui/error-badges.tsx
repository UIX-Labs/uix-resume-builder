...
export function ErrorBadge({ count, type, className, onClick }: ErrorBadgeProps) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
    >
      <span className="text-[8px] sm:text-[10px] flex-shrink-0">{badge.icon}</span>
...
export function FieldErrorBadges({
  ...
  return (
    <div className={cn('flex gap-1.5 sm:gap-2 flex-wrap min-w-0', className)}>
      <ErrorBadge count={spellingCount} type={SuggestionType.SPELLING_ERROR} onClick={onBadgeClick} />
      <ErrorBadge count={sentenceCount} type={SuggestionType.SENTENCE_REFINEMENT} onClick={onBadgeClick} />
      <ErrorBadge count={newSummaryCount} type={SuggestionType.NEW_SUMMARY} onClick={onBadgeClick} />
    </div>
  );
}
