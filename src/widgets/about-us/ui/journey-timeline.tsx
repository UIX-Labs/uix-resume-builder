...
export function JourneyTimeline() {
	...
	useEffect(() => {
		...
		const cumulativeLengths = pathLengths.reduce<number[]>(
			(acc, length, index) => {
				...
				threadPathRefs.current.forEach((path, index) => {
					if (path && pathLengths[index]) {
						...
						path.style.strokeDashoffset = `${
							pathLengths[index] - pathProgress
						}`;
					}
				});
			...
			if (mobileThreadRef.current && window.innerWidth < 1024) {
				...
				if (mobileContainer) {
					...
					const scrollPercentage = Math.max(
						0,
						Math.min(scrolled / Math.max(totalScrollable, 1), 1)
					);

					const mobileLength = mobileThreadRef.current.getTotalLength();
					const drawAmount = mobileLength * scrollPercentage;
					mobileThreadRef.current.style.strokeDashoffset = `${
						mobileLength - drawAmount
					}`;
