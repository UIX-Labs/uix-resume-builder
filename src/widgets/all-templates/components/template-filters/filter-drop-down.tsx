  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className="flex items-center justify-between min-w-[140px] rounded-xl px-4 py-3 font-medium text-gray-700 border-gray-300 gap-6"
        >
          <div className="flex items-center gap-2">
            {selectedValues.length > 0 ? (
              <>
                ...
                {selectedValues.length > 1 && <Badge variant="secondary">+{selectedValues.length - 1}</Badge>}
              </>
            ) : (
              <span>{label}</span>
            )}
