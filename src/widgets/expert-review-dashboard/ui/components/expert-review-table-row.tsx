...
export function ExpertReviewTableRow({ review }: ExpertReviewTableRowProps) {
  ...
  return (
    <tr className="border-b border-gray-100 last:border-0 hover:bg-gray-50/50 transition-colors">
      <td className="px-4 sm:px-6 py-3 font-medium text-neutral-900 truncate max-w-[200px]">
        {review.name}
      </td>
      ...
      <td className="px-4 sm:px-6 py-3 text-[#959DA8] hidden sm:table-cell">
        {review.submitted}
      </td>
      <td className="px-4 sm:px-6 py-3 text-[#959DA8] hidden lg:table-cell">
        {formatDateShort(review.lastModified)}
      </td>
      <td className="px-4 sm:px-6 py-3">
        {isReviewDone && (
          <button
